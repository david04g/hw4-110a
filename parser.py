...
class Parser:
    ...
    def parse_statement_list(self) -> List[str]:
        ...
        if token_id in [Token.INT, Token.FLOAT, Token.ID, Token.IF, Token.LBRACE, Token.FOR]:
            # Recursively parse a single statement
            lines = self.parse_statement()
            # Parse the rest of the statements in the block
            lines += self.parse_statement_list()
            # Return all instructions
            return lines
        if token_id in [Token.RBRACE]:
            # End of block, return empty list
            return []
        
    # you need to return a list of three address instructions
    # from the statement that gets parsed
    # Parses the parse statement
    def parse_statement(self) -> List[str]:
        ...
        if token_id in [Token.INT, Token.FLOAT]:
            ...
            # Error if unrecognized statement
            raise ParserException(self.scanner.get_lineno(),
                                  self.to_match,
                                  [Token.FOR, Token.IF, Token.LBRACE, Token.INT, Token.FLOAT, Token.ID])

    # you need to return a list of three address instructions
    # Parses a declaration like `int x;` or `float y;`
...
# Parses assignment like `x = y + 1;`
    def parse_assignment_statement(self) -> List[str]:
        lines = self.parse_assignment_statement_base()  # Parse RHS
        self.eat(Token.SEMI)  # End of statement
        return lines
    ...
    # Parses an expression with possible == comparison
    def parse_expr(self) -> ASTNode:
        left = self.parse_comp()
        return self.parse_expr2(left)

    # Parses == continuation of an expression
    ...
    # Parses a comparison expression with <
    def parse_comp(self) -> ASTNode:
        left = self.parse_factor()
        return self.parse_comp2(left)

    # Parses continuation of comparison (e.g., <)
    ...
    # Parses an additive expression (left + right)
    def parse_factor(self) -> ASTNode:
        left = self.parse_term()
        return self.parse_factor2(left)

    # Parses continuation of addition/subtraction
    ...
    # Parses a term (like left * right or left / right)
    def parse_term(self) -> ASTNode:
        left = self.parse_unit()
        return self.parse_term2(left)

    # Parses continuation of multiplication/division
    ...
    # Parses a number, identifier, or parenthesized expression
...
# Checks if a node is a leaf (constant or variable)
def is_leaf_node(node) -> bool:
    return issubclass(type(node), ASTLeafNode)

# Infers and assigns a type to an expression node
